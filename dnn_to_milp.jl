...
function create_empty_biases(num_layers::Int, num_nodes_in_layer::Vector{Int})::Dict
    ...
    for l in 1:num_layers 
        biases[l] = Dict() 
        for i in 1:num_nodes_in_layer[l]
            biases[l][i] = 0.0
        end 
    end 
    return biases
...
function get_milp(num_layers::Int, num_nodes_in_layer::Vector{Int}, w::Dict, b::Dict; m = JuMP.Model())
    Mp = 1e6 
    Mn = 1e6 
    ...
    # linking constraints 
    @constraint(m, z1_implies_x0[l in 2:(num_layers-1), n in 1:num_nodes_in_layer[l]], x[l, n] <= Mp * (1 - z[l, n]))
    @constraint(m, z0_implies_s0[l in 2:(num_layers-1), n in 1:num_nodes_in_layer[l]], s[l, n] <= Mn * z[l, n])

    # layer connections intermediate layers 
    @constraint(m, layer_connections[l in 2:(num_layers-1), n in 1:num_nodes_in_layer[l]], 
        sum(w[l-1][(n, j)] * x[l-1, j] for j in 1:num_nodes_in_layer[l-1]) + b[l][n] == 
        x[l, n] - s[l, n]
    )

    # layer connection last layer 
    @constraint(m, layer_connections_last, 
        sum(w[num_layers - 1][(1, j)] * x[num_layers - 1, j] for j in 1:num_nodes_in_layer[num_layers - 1]) + 
